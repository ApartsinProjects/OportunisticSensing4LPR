...
def convert_html_to_docx(input_file, output_file, profile="camera-ready-generic"):
    ...
    # --resource-path tells Pandoc where to look for image files whose src=
    # is given as a relative path; since strip_html_title_tag writes the
    # sanitised copy to a temp directory, we must point Pandoc back at the
    # original input's directory so relative image references resolve.
    temp_input = strip_html_title_tag(input_file)
    original_dir = str(Path(input_file).resolve().parent)
    try:
