...
def _merge_equation_numbers(doc, right_tab_twips=9360):
    ...
    while i < len(children) - 1:
        p = children[i]
        nxt = children[i + 1]
        if p.tag != qn("w:p") or nxt.tag != qn("w:p"):
            i += 1
            continue
        math_para = p.find(f"{{{MATH_NS}}}oMathPara")
        if math_para is None:
            i += 1
            continue
        # Check the next paragraph text is exactly "(N)"
        text_parts = nxt.findall(f".//{{{W_NS}}}t")
        text = "".join((t.text or "") for t in text_parts).strip()
        m = _EQ_NUM_RE.match(text)
        if not m:
            i += 1
            continue
        number_text = f"({m.group(1)})"

        # Unwrap <m:oMath> out of <m:oMathPara> so it becomes an inline
        # math element directly inside the <w:p>.
        omath = math_para.find(f"{{{MATH_NS}}}oMath")
        if omath is None:
            i += 1
            continue
        ...
        body.remove(nxt)
        # Rebuild the children view and continue from the same index since
        # we removed nxt (the list shrinks by one).
        children = list(body.iterchildren())
        merged += 1
        # do not advance i; the next paragraph shifted into position i+1
    if merged:
        print(f"  Merged {merged} equation numbers onto their equation lines")
    return merged
